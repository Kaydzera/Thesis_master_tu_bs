...
\textbf{Remark:} The proof above follows the approach of~\cite{weninger2025bilevel}, who proved an analogous result for bilevel min-max problems. While their setting considers minimization in the upper level and maximization in the lower level, the proof structure is practically identical to our max-min case, with the inequality directions appropriately reversed.
...
We exploit this property by considering each item type $J$ as a candidate for the job that finishes last. For a fixed candidate $J$, we maximize the makespan of the remaining jobs under the remaining budget.
...
\textbf{Connection to unbounded knapsack:} By allocating budget $B_i$ to each machine, we seek the combination of job types that maximizes the total duration (load) on each machine given this budget constraint. This is precisely the unbounded knapsack problem: maximize $\sum_{i=1}^{J} d_i x_i$ subject to $\sum_{i=1}^{J} p_i x_i \leq B_i$, where we can select multiple copies of each job type. The dynamic programming table $DP(J, B_i)$ stores the optimal value of this knapsack problem, i.e., the maximum load achievable on a single machine with budget $B_i$ using job types $1, \ldots, J$. Since the LPT rule places the last job of type $J$ on the least loaded machine, maximizing the load on all machines before this placement corresponds to maximizing $C_{\max}^{(m)}$, the minimum machine load at that moment.

If $\hat{B} \ge p_J$, the final job of type $J$ can be paid without reducing the per-machine budget. In this case, the optimal combination of jobs scheduled before $J$ is given by the unbounded knapsack solution stored in $DP(J, B_i)$. This corresponds to the case where no adjustment is needed in the algorithm.

If $\hat{B} < p_J$, the per-machine budget $B_i$ must be reduced uniformly until the remaining budget is sufficiently large to pay for item $J$. After this adjustment, the optimal combination of jobs scheduled before $J$ is again obtained from the dynamic programming table $DP(J, B_i)$. This corresponds to the adjustment loop in the algorithm.
...
We have already shown that this approach yields a $\frac{3}{4}$-approximation for the original knapsack-scheduling problem (Section~\ref{sec:formulation}). If we solve the max-LPT problem before calling the branch and bound Algorithm we use to obtain an optimal solution to the bilevel knapsack-scheduling problem, we obtain a starting incumbent that is guaranteed to be within $\frac{3}{4}$ of the optimal solution. 
