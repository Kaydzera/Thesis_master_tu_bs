...
While the branch-and-bound algorithm explores the search space systematically with pruning, we now present an alternative approach that directly maximizes the LPT makespan. This algorithm provides another method to solve the knapsack-scheduling problem, and we will later compare both approaches experimentally in Chapter~\ref{sec:experiments}.
...
We apply Theorem~\ref{thm:approx-bilevel} to our knapsack-scheduling problem. The LPT rule is known to be a $\frac{4}{3}$-approximation algorithm for the makespan minimization problem on parallel machines. By Lemma~\ref{lem:approx-properties}, this means that for any job selection $x$, the optimal makespan satisfies $\frac{4}{3} \cdot C_{\max}^{\text{OPT}}(x) \geq C_{\max}^{\text{LPT}}(x)$.

Now, let $\hat{x}$ be the solution that maximizes the LPT makespan, i.e., $\hat{x}$ solves the Max-LPT problem. Since the leader seeks to maximize (not minimize) makespan, we adapt Theorem~\ref{thm:approx-bilevel} to the maximization setting. The optimal solution $x^*$ maximizes the optimal makespan, and we have:
\[
C_{\max}^{\text{LPT}}(\hat{x}) \geq C_{\max}^{\text{LPT}}(x^*) \geq \frac{3}{4} \cdot C_{\max}^{\text{OPT}}(x^*) = \frac{3}{4} \cdot C_{\max}^{\text{OPT}},
\]
where the second inequality follows from the $\frac{4}{3}$-approximation guarantee of LPT (equivalently, $C_{\max}^{\text{OPT}} \leq \frac{4}{3} \cdot C_{\max}^{\text{LPT}}$, which gives $C_{\max}^{\text{LPT}} \geq \frac{3}{4} \cdot C_{\max}^{\text{OPT}}$).
\end{proof}
