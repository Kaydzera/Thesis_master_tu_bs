...
With the search tree structure, valid upper bounds, and pruning conditions established, we can now present the complete branch-and-bound algorithm. Algorithm~\ref{alg:bnb} integrates all components: it systematically explores the search tree, computes upper bounds at each node, and prunes branches when the bound indicates that no improvement over the incumbent is possible. The algorithm maintains a queue of unexplored nodes and continues until all promising branches have been exhausted.
...
\[
\frac{3}{4} \cdot C_{\max}^{\text{OPT}} \leq C_{\max}^{\text{LPT}}.
\]
\end{theorem}
...
\label{alg:maxLPT}
\begin{algorithmic}[1]
\Require Budget $B$, item types $1, \ldots, n$ with durations $d_i$ and costs $p_i$, number of machines $m$
...
\For{each item $J \in \{1,\ldots,n\}$}
    ...
    \If{$\hat{B} < p_J$}
        ...
        \While{$\hat{B} < p_J$ \textbf{and} $B_i > 0$}
            ...
            \State $\hat{B} := B - m \cdot B_i$
            \If{$B_i < 0$ \textbf{or} $\hat{B} < 0$}
                \State \textbf{break} \Comment{Safety check: prevent infinite loop}
            \EndIf
        \EndWhile
        \If{$\hat{B} < p_J$}
...
Since we assume to have an optimal solution, there cannot be a combination of jobs so that the LPT rule would deliver a larger $C_{\max}^{(m)}$ for the unbounded knapsack subproblem that maximizes $C_{\max}^{(m)}$ with reduced budget (by how much, we will explore in the following) and job types $1, \ldots, I$ available.
Otherwise, we could replace the combination of jobs in the optimal solution before putting $I$ on the least loaded machine with the combination that maximizes $C_{\max}^{(m)}$ and then place item $I$ on the least loaded machine, which contradicts the optimality of the solution.
This means that all we need to do is maximize $C_{\max}^{(m)}$ for the unbounded knapsack subproblem with budget $B - p_I$ for all job types $1, \ldots, I$ available for each candidate $I$ for the last job.
...
If $\hat{B} \ge p_J$, the final job of type $J$ can be paid without reducing the per-machine budget. This corresponds to the case where no adjustment is needed in the algorithm. In this case, the optimal combination of jobs scheduled before $J$ is given by the unbounded knapsack solution stored in $DP(J, B_i)$. 
This follows because we evenly distribute the budget across all machines, thus maximizing the budget the least loaded machine has on its disposal. Given this Budget and items $1, \ldots, J$, we seek the combination of items that maximizes the sum of durations on each machine. This is equivalent to solving the unbounded knapsack problem with the dynamic programming table $DP(J, B_i)$, which stores the optimal value of this knapsack problem for item types $1, \ldots, J$ and budget $B_i$. Adding the duration of the final job $d_J$ gives us the total makespan for this candidate solution.

If $\hat{B} < p_J$, the per-machine budget $B_i$ must be reduced uniformly until the remaining budget is large enough to pay for item $J$. After this adjustment, the optimal combination of jobs scheduled before $J$ is again obtained from the dynamic programming table $DP(J, B_i)$. This corresponds to the adjustment loop in the algorithm.
...
\State Initialize: $(x^*, C_{\max}^*) \gets$ Max-LPT solution, queue $Q \gets \{$root node$\}$
\While{$Q \neq \emptyset$}
    \State $x \gets Q.\text{pop}()$ \Comment{Node with partial solution at depth $d$}
    \If{$x$ is complete solution}
        \State Solve follower's problem: $C_{\max}(x)$
