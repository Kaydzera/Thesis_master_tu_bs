...
% Root node
\node[node] (root) {\textbf{Root}\\$d=0$\\$x=\emptyset$\\$B_{\text{rem}}=10$}
...
To evaluate partial solutions in the branch-and-bound tree, we need to compute the makespan for the jobs that have already been selected. For this purpose, we use the \textbf{Longest Processing Time (LPT)} rule~\cite{williamson2011design}, a well-known greedy algorithm for scheduling jobs on parallel machines. 
