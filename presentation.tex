...
\begin{frame}[shrink=5]{Bilevel Optimisation: Hardness}
  \begin{alertblock}{Computational difficulty}
    Bilevel optimisation is NP-hard in general~\cite{frangioni1995}.
  \end{alertblock}
  \begin{block}{Our specific challenge}
    \begin{itemize}
      \item \textbf{Leader} solves an unbounded knapsack (NP-hard)
...
\begin{frame}[shrink=5]{The Knapsack-Scheduling Problem}
  ...
  \begin{block}{Complete Bilevel Formulation}
    ...
    \begin{align*}
      ...
      \text{s.t.}\quad & \textstyle\sum_{k=1}^{m}y_{ik}=x_i\quad\forall\,i
    \end{align*}
    \vspace{-0.3em}
  \end{block}
  \smallskip
...
\begin{frame}[shrink=8]{Branch-and-Bound: Overview}
  ...
  \begin{columns}[T]
    \begin{column}{0.47\textwidth}
      ...
      \begin{enumerate}
        \item \textbf{Branching} — fix $x_{d+1}$
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.44\textwidth}
      \begin{block}{LPT algorithm}
        ...
        \textbf{for} each job $j$ (desc.\ order):\\
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{Max-LPT bound}
        $\Delta = \Clpt(y_{d+1},\ldots) - C_{\max}^{\mathrm{LPT},(d)}$\\
...
\begin{frame}[shrink=8]{Approximation Guarantee: Building Blocks}
  \begin{block}{Lemma (Structure of $\alpha$-approximations)}
    \vspace{-0.2em}
    \small Let $\Clpt$ be a $\frac{4}{3}$-approx.\ for minimization. Then for all $x$:
    \[\tfrac{3}{4}\,\Copt(x)\;\leq\;\tfrac{3}{4}\,\Clpt(x)\;\leq\;\Copt(x)\;\leq\;\Clpt(x)\]
    \vspace{-0.3em}
  \end{block}
  \begin{block}{Theorem (Bilevel Approx.\ [Weninger \& Fukasawa 2025])}
    \vspace{-0.2em}
    \small If $\beta\,\Copt(x)\leq\Clpt(x)$ and $\hat x\in\arg\max_x\Clpt(x)$, then
    \[\beta\cdot\mathrm{Opt}_{\text{bilevel}}\;\leq\;\Copt(\hat x)\;\leq\;\mathrm{Opt}_{\text{bilevel}}\]
    \vspace{-0.3em}
  \end{block}
  \textbf{Combined:} with $\beta=\tfrac{3}{4}$ (LPT guarantee), solving Max-LPT gives an incumbent
  within $25\%$ of optimal — \emph{before any B\&B search}.
\end{frame}
...
\begin{frame}[shrink=5]{The $\frac{3}{4}$-Approximation Guarantee}
  ...
  \textbf{Proof:}
  \begin{itemize}
    \item By LPT guarantee: $\tfrac34\,\Copt(x)\leq\tfrac34\,\Clpt(x)\leq\Copt(x)\leq\Clpt(x)$ for all $x$
    \item $\hat x$ maximises $\Clpt$, so $\Clpt(x^*)\leq\Clpt(\hat x)$
    \item Chain: $\tfrac34\,\Copt(x^*)\leq\tfrac34\,\Clpt(x^*)\leq\tfrac34\,\Clpt(\hat x)\leq\Copt(\hat x)\leq\Copt(x^*)$ \hfill$\square$
  \end{itemize}
...
\begin{frame}[shrink=5]{Max-LPT Algorithm}
  \textbf{Key observation:} the \emph{last} scheduled job lands on the least-loaded machine.
  \[\Clpt = C_{\max}^{(m)} + d_I
    \quad\text{where $C_{\max}^{(m)}$ = $m$-th largest load, $I$ = last job}\]
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      \textbf{MAX-LPT algorithm:}\\[0.15em]
      \small For each candidate last job $J$:
      \begin{enumerate}\small
        \item $B^*\leftarrow\lfloor B/m\rfloor$ (per-machine budget)
        \item While $(B-mB^*)<p_J$: $B^*\leftarrow B^*-1$
        \item $z\leftarrow\mathrm{DP}(J,B^*)+d_J$
      \end{enumerate}
      \normalsize Return $z^*=\max_J z$\quad [$O(n\cdot B)$]
    \end{column}
    \begin{column}{0.47\textwidth}
      \begin{block}{Correctness}
        \small $B^*$: max per-machine budget for the least-loaded machine without job $J$'s cost.
        $\mathrm{DP}(J,B^*)$: max load on that machine.
        Adding $d_J$ gives the optimum makespan for this candidate last job.
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=5]{Early Optimality Detection}
  ...
  \begin{alertblock}{Impact}
    \begin{itemize}
      ...
      \item Triggered in $\approx\mathbf{84.4\%}$ of our test instances $\Rightarrow$ solved at root
    \end{itemize}
  \end{alertblock}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.53\textwidth}
      ...
      \begin{enumerate}
        ...
        \item \textbf{If} $\Clpt(\hat x)=\Copt(\hat x)$: \textbf{return} $\hat x$
              \hfill{\footnotesize (84\% of instances)}
        \item $\Cinc\leftarrow\Copt(\hat x)$;\quad push root to stack $Q$
        \item \textbf{While} $Q\neq\emptyset$:
        \begin{itemize}\small
          \item Pop node $(d,x_1,\ldots,x_d,\Brem)$
    ...
    \begin{column}{0.44\textwidth}
      ...
      \begin{block}{Follower solver}
        \small Gurobi 12.0.3 solves the MIP formulation of $P||\Cmax$ at leaves.
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=8]{Grid Sensitivity Analysis}
  \textbf{Grid:} $6{\times}6$ (types $\times$ machines), 3 budget multipliers, 5 seeds $=540$ instances; 1\,h limit.
...
\begin{frame}[shrink=5]{Comparison with MibS}
  ...
  \begin{alertblock}{Speed-up over MibS (1\,h limit)}
    \small Ceiling: mean $+2{,}877$\,s, median $+3{,}600$\,s\quad
    MaxLPT: mean $+2{,}960$\,s, median $+3{,}600$\,s\\
  ...
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      ...
      \begin{enumerate}
        ...
        \item Ceiling bound with precomputed $O(1)$ DP
        \item Max-LPT solved optimally in $O(n\cdot B)$
        \item $\tfrac34$-approximation guarantee (theory)
        \item Early optimality detection ($84\%$ solve at root)
    ...
    \begin{column}{0.42\textwidth}
      \begin{alertblock}{Limitations}
        ...
        \begin{itemize}
          ...
          \item Exponential worst-case for $>30$ types
